...
void main() {
	...
	char option_c = 1;
	char argPath_ca[MAX_PATH] = "C:\\Windows\\System32\\calc.exe";													// Full file path
	...
	while (relocTblAdr_ll != 0) {
		...
		if (relocBlokSize_i >= 8) {												// Block size must be > size of struct _IMAGE_BASE_RELOCATION in order to have any Descriptor
			...
			for (int ct = 0; ct < relocNbDesc_i; ct++) {
				...
				locVirtRelocTblAdr_ll += 2;
			}
		}									// Go to next descriptor
		relocTblAdr_ll = *(int*)locVirtRelocTblAdr_ll;
...
void PEInject(char argPath_ca[], char* option_p, struct locVirtAlloc_stc* locVirtAlloc, struct remVirtAlloc_stc* remVirtAlloc) {
	...
	WriteProcessMemory(remVirtAlloc->process_vp, (char*)cmdBase_vp + MAX_PATH, option_p, sizeof(char), NULL);	// Copy the path to the file to pin to taskbar, into the extra memory of size "MAX_PATH"
// Run the "PinToTaskBar_func" in the "Progman" process, with the path to the file to pin to taskbar as a parameter
	LPTHREAD_START_ROUTINE startRoutine_lptsr = (LPTHREAD_START_ROUTINE)(remVirtAlloc->relocOffset_ll + (char*)PinToTaskBar_func);
...
void GetPinVerbs(char option_c, wchar_t pttbVerb_wca[], wchar_t upftbVerb_wca[], wchar_t* pinVerbs[]) {
	...
	if (option_c != ONLY_UNPIN) {
		LoadStringW(GetModuleHandleW(L"shell32.dll"), 5386, pttbVerb_wca, MAX_PATH);
	} // 5386: "Pin to tas&kbar" in en-us locale versions of Windows
	else {
		pttbVerb_wca = NULL;
	}
	FreeLibrary((HMODULE)shell32_vp);
